...
def sort_files(files: List[str]) -> List[str]:
  """makes sure the timestamps are sorted - latest first"""
...
def play_selected(
  allFiles: List[str], original_files: Dict[str, str], selected: str) -> None:
  """play the files corresponding to the selected timestamp"""
  for file in allFiles:
    if original_files[selected] in file:
      if "TOP_VIEW" in file:
        os.system(f"mpv --really-quiet {directory / file} --video-rotate=270 &")
      else:
        os.system(f"mpv --really-quiet {directory / file} &")
  print(f"Playing {selected}")
  time.sleep(15)

def delete_selected(allFiles: List[str], file_mapping: Dict[str, str], option: str) -> None:
  """Delete the selected files"""
  for file in allFiles:
    if file_mapping[option] in file:
      os.remove(directory / file)
      print(f"Deleted {file}")

# Prompt the user to play or delete the selected files
def play_or_delete(allFiles: List[str], file_mapping: Dict[str, str], option: str) -> None:
  """Prompt the user to play or delete the selected files"""
  title = "What do you want to do?"
  options = ["Play", "Delete"]
  choice, _ = pick.pick(options, title)
  if choice == "Play":
    play_selected(allFiles, file_mapping, option)
  elif choice == "Delete":
    delete_selected(allFiles, file_mapping, option)
...
def main():
  ...
  play_or_delete(allFiles, file_mapping, option)
